    ...
    public function viewAllBlotters(Request $request)
{
    ...
    if ($request->has('status')) {
        ...
        // Make sure the status is a valid value (0-3) to avoid invalid SQL injections
        if (in_array($status, [0, 1, 2, 3])) {
            $status_filter = "AND br.status_resolved = $status";
        }
    }

    // Combine all filters
    $where_clause = "WHERE br.id IS NOT NULL $search_value $date_filter $category_filter $status_filter";
